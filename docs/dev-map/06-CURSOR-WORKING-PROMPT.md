...
4. P0 已完成后：只把功能填进现有名词，遵守 07 的模块数锁定与落点顺序；不得为小功能新建模块。AI 已解冻：只走现有 `IHttpClient` / AI 模块与 `skill.json`，禁止供应商 SDK 与密钥进日志。仍处于某 Phase 时：只实现该 Phase 允许的内容。请求超出范围时，先指出冲突；没有产品负责人明确批准，不得跳 Phase、扩大 P0 或解开 07 的模块冻结。
