...
> 权威关系：`../00`–`../08` 仍是最高约束（依赖方向、模块数锁定、AI 冻结、UI 只发 Command）。`../ui-pro-upgrade/` 仍管界面主次与区域 ID。本文件夹只在 [`35-DO-NOT.md`](35-DO-NOT.md) 第四节列出的几处**明确改写**既有决策，其余一律服从。
...
| 目标版本 | 0.1.3（止血）→ 0.2（地基）→ 0.3（镜头包）→ 0.4（Skills）→ 0.5（角色包与体验，待批准） |
| 允许改 | 所有已有模块内部；`Core::Command` 新增值对象；`AppViewState` 新增只读字段；`App` 目标内**分文件**；`modules/*.md` 契约升小版本 |
| 不允许改 | 模块集合（不新建 `src/<Module>/` 与 CMake 目标）；依赖方向；AI 冻结（不接线真实服务、不跑子进程）；第三方类型进公共头 |
...
| **F4 · 角色包与体验** | 0.5 | 角色资产包（含骨骼字段的 manifest）静态加载；Gizmo；Undo | **待批准**：涉及第三方依赖与 `00` 红线 |
