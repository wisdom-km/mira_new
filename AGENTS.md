...
- 第三方类型不进公共业务头
- AI 已解冻：只走 `IHttpClient` 与现有 AI 模块，不得引入供应商 SDK，不得把密钥写入日志
